# app_name = "pc_production"
# app_title = "PC Production"
# app_publisher = "TRPL Solutions"
# app_description = "Perfect Craft Production Costing"
# app_email = "jaweriabibi098@gmail.com"
# app_license = "mit"


# auto_cancel_exempted_doctypes = [
#     "Purchase Invoice"
# ]


# fixtures = [

#     {
#         "dt": "Custom Field",
#         "filters": [
#             [
#                 "dt",
#                 "in",
#                 [
#                     "Stock Reconciliation",
#                     "Stock Reconciliation Item",
#                     "Delivery Trip",
#                 ],
#             ],
#             [
#                 "fieldname",
#                 "in",
#                 [
#                     "custom_expected_per_qty_amount",
#                     "custom_actual_qty",
#                     "custom_adjustment_per_qty",
#                     "custom_production_expenses",
#                     "custom_item_valuation_rate",
#                     "custom_transporter",
#                     "custom_supplier_name",
#                     "custom_address",
#                     "custom_gate_pass_date",
#                     "custom_remarks",
#                     "custom_delivery_charges",
#                     "custom_purchase_invoice",
#                 ],
#             ],
#         ],
#     },


#     # --------------------------------------------------------
#     # Purchase Invoice -> Gate Pass reference
#     # --------------------------------------------------------

#     {
#         "dt": "Custom Field",
#         "filters": [
#             [
#                 "dt",
#                 "=",
#                 "Purchase Invoice",
#             ],
#             [
#                 "fieldname",
#                 "=",
#                 "custom_gate_pass",
#             ],
#         ],
#     },


#     {
#         "dt": "Property Setter",
#         "filters": [
#             [
#                 "doc_type",
#                 "=",
#                 "Stock Reconciliation Item",
#             ],
#             [
#                 "field_name",
#                 "=",
#                 "valuation_rate",
#             ],
#             [
#                 "property",
#                 "=",
#                 "label",
#             ],
#         ],
#     },

# ]


# # ============================================================
# # DOCTYPE JS
# # ============================================================

# doctype_js = {

#     "Stock Entry":
#         "public/js/stock_entry.js",

#     "Stock Reconciliation":
#         "public/js/stock_reconciliation.js",

#     "BOM":
#         "pc_production/doctype/bom/bom.js",

#     # Same file contains both Delivery Note calculation
#     # and Delivery Trip total calculation.
#     "Delivery Note":
#         "pc_production/doctype/delivery_note/delivery_note.js",

#     "Delivery Trip":
#         "pc_production/doctype/delivery_note/delivery_note.js",

# }


# # ============================================================
# # OVERRIDE DOCTYPE CLASS
# # ============================================================

# override_doctype_class = {

#     "Stock Entry":
#         "pc_production.stock_entry.CustomStockEntry",

# }


# # ============================================================
# # DOCUMENT EVENTS
# # ============================================================

# doc_events = {


#     # --------------------------------------------------------
#     # Delivery Note
#     # --------------------------------------------------------

#     "Delivery Note": {

#         "before_validate":
#             "pc_production.pc_production.doctype.delivery_note.delivery_note.calculate_delivery_note_amount",

#         "before_update_after_submit":
#             "pc_production.pc_production.doctype.delivery_note.delivery_note.calculate_delivery_note_amount",

#     },


#     # --------------------------------------------------------
#     # Delivery Trip
#     # --------------------------------------------------------

#     "Delivery Trip": {

#         # Delivery Charges =
#         # sum of linked Delivery Note calculated amounts
#         "before_validate":
#             "pc_production.pc_production.doctype.delivery_note.delivery_note.calculate_delivery_trip_charges",


#         # Existing Purchase Invoice automation
#         "on_submit":
#             "pc_production.delivery_trip.create_purchase_invoice",

#         "on_cancel":
#             "pc_production.delivery_trip.cancel_purchase_invoice",

#         "on_trash":
#             "pc_production.delivery_trip.delete_purchase_invoice",

#     },


#     # --------------------------------------------------------
#     # Stock Entry
#     # --------------------------------------------------------

#     "Stock Entry": {

#         "before_validate":
#             "pc_production.stock_entry.apply_monthly_production_overhead"

#     },


#     # --------------------------------------------------------
#     # Stock Reconciliation
#     # --------------------------------------------------------

#     "Stock Reconciliation": {

#         "before_validate":
#             "pc_production.stock_reconciliation.apply_stock_reconciliation_calculation",

#         "before_submit":
#             "pc_production.stock_reconciliation.validate_stock_reconciliation",

#     },


#     # --------------------------------------------------------
#     # BOM
#     # --------------------------------------------------------

#     "BOM": {

#         "before_validate":
#             "pc_production.pc_production.doctype.bom.bom.sync_raw_material_and_scrap"

#     },

# }


# # ============================================================
# # PRINT FORMATS
# # ============================================================

# jinja = {

#     "methods": [

#         "pc_production.print_utils.get_sales_order_print_context",

#         "pc_production.print_utils.get_delivery_trip_print_context",

#         "pc_production.print_utils.get_gate_pass_print_context",

#         "pc_production.sales_invoice_print.get_sales_invoice_print_context",

#     ]

# }


# # ============================================================
# # AFTER MIGRATE
# # ============================================================

# after_migrate = [

#     # BOM schema
#     "pc_production.pc_production.doctype.bom.bom.after_migrate",

#     # Remove previous incorrect Delivery Stop customization
#     "pc_production.pc_production.doctype.delivery_note.delivery_note.after_migrate",

#     # Existing print formats
#     "pc_production.print_utils.ensure_sales_order_print_format",

#     "pc_production.print_utils.ensure_delivery_trip_print_format",

#     "pc_production.print_utils.ensure_gate_pass_print_format",

#     "pc_production.sales_invoice_print.ensure_sales_invoice_print_format",

# ]











app_name = "pc_production"
app_title = "PC Production"
app_publisher = "TRPL Solutions"
app_description = "Perfect Craft Production Costing"
app_email = "jaweriabibi098@gmail.com"
app_license = "mit"


auto_cancel_exempted_doctypes = [
    "Purchase Invoice"
]


fixtures = [
    {
        "dt": "Custom Field",
        "filters": [
            [
                "dt",
                "in",
                [
                    "Stock Reconciliation",
                    "Stock Reconciliation Item",
                    "Delivery Trip",
                ],
            ],
            [
                "fieldname",
                "in",
                [
                    "custom_expected_per_qty_amount",
                    "custom_actual_qty",
                    "custom_adjustment_per_qty",
                    "custom_production_expenses",
                    "custom_item_valuation_rate",
                    "custom_transporter",
                    "custom_supplier_name",
                    "custom_address",
                    "custom_gate_pass_date",
                    "custom_remarks",
                    "custom_delivery_charges",
                    "custom_purchase_invoice",
                ],
            ],
        ],
    },

    {
        "dt": "Custom Field",
        "filters": [
            ["dt", "=", "Purchase Invoice"],
            ["fieldname", "=", "custom_gate_pass"],
        ],
    },

    {
        "dt": "Property Setter",
        "filters": [
            ["doc_type", "=", "Stock Reconciliation Item"],
            ["field_name", "=", "valuation_rate"],
            ["property", "=", "label"],
        ],
    },
]


doctype_js = {
    "Stock Entry": "public/js/stock_entry.js",
    "Stock Reconciliation": "public/js/stock_reconciliation.js",
    "BOM": "pc_production/doctype/bom/bom.js",

    "Delivery Trip":
        "pc_production/doctype/delivery_trip/delivery_trip.js",
}


override_doctype_class = {
    "Stock Entry":
        "pc_production.stock_entry.CustomStockEntry",
}


doc_events = {

    "Delivery Trip": {

        "before_validate":
            "pc_production.pc_production.doctype.delivery_trip.delivery_trip.calculate_delivery_charges",

        # Existing PI automation - unchanged
        "on_submit":
            "pc_production.delivery_trip.create_purchase_invoice",

        "on_cancel":
            "pc_production.delivery_trip.cancel_purchase_invoice",

        "on_trash":
            "pc_production.delivery_trip.delete_purchase_invoice",
    },


    "Stock Entry": {
        "before_validate":
            "pc_production.stock_entry.apply_monthly_production_overhead"
    },


    "Stock Reconciliation": {
        "before_validate":
            "pc_production.stock_reconciliation.apply_stock_reconciliation_calculation",

        "before_submit":
            "pc_production.stock_reconciliation.validate_stock_reconciliation",
    },


    "BOM": {
        "before_validate":
            "pc_production.pc_production.doctype.bom.bom.sync_raw_material_and_scrap"
    },
}


jinja = {
    "methods": [
        "pc_production.print_utils.get_sales_order_print_context",
        "pc_production.print_utils.get_delivery_trip_print_context",
        "pc_production.print_utils.get_gate_pass_print_context",
        "pc_production.sales_invoice_print.get_sales_invoice_print_context",
    ]
}


after_migrate = [

    "pc_production.pc_production.doctype.bom.bom.after_migrate",

    "pc_production.pc_production.doctype.delivery_trip.delivery_trip.after_migrate",

    "pc_production.print_utils.ensure_sales_order_print_format",

    "pc_production.print_utils.ensure_delivery_trip_print_format",

    "pc_production.print_utils.ensure_gate_pass_print_format",

    "pc_production.sales_invoice_print.ensure_sales_invoice_print_format",
]